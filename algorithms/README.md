...
- **`algorithm.ts`** — Export a generator that yields `AlgorithmStep<TData>` with `data`, `codeRange`, `description`, and optional `variables`. Use `createStep()` if you like. The `codeRange` line numbers must match the code file.
